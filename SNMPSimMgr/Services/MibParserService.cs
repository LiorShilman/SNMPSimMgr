...
namespace SNMPSimMgr.Services
{
    public static class MibParserService
    {
        ...
        private static readonly Dictionary<string, string>  RootOids = new Dictionary<string, string>() {
            ["iso"] = "1",
            ["org"] = "1.3",
            ["dod"] = "1.3.6",
            ["internet"] = "1.3.6.1",
        ...
        private static readonly Regex OidAssignRegex = new Regex(
            @"::=\s*\{([^}]+)\}",
            RegexOptions.Compiled);

        // Simpler form: name OBJECT IDENTIFIER ::= { ... }  (single or multi-line)
        private static readonly Regex SimpleOidRegex = new Regex(
        ...
        private static readonly Regex AccessRegex = new Regex(
        ...
        private static readonly Regex UnitsRegex = new Regex(
            @"UNITS\s+""([^""]*)""",
            RegexOptions.Compiled);

        private static readonly Regex DefValRegex = new Regex(
            @"DEFVAL\s*\{\s*([^}]*)\s*\}",
            RegexOptions.Compiled);

        private static readonly Regex DisplayHintRegex = new Regex(
            @"DISPLAY-HINT\s+""([^""]*)""",
            RegexOptions.Compiled);

        private static readonly Regex IndexPartsRegex = new Regex(
            @"INDEX\s*\{\s*([^}]+)\s*\}",
            RegexOptions.Compiled);

        // Boundary: detects the start of the NEXT definition (to limit defRegion scope)
        private static readonly Regex NextDefinitionRegex = new Regex(
        ...
        // Syntax sub-patterns
        private static readonly Regex EnumValuesRegex = new Regex(
            @"\{\s*((?:\w[\w-]*\s*\(\s*-?\d+\s*\)\s*,?\s*)+)\}",
            RegexOptions.Compiled);

        private static readonly Regex SingleEnumRegex = new Regex(
            @"(\w[\w-]*)\s*\(\s*(-?\d+)\s*\)",
            RegexOptions.Compiled);

        private static readonly Regex IntRangeRegex = new Regex(
            @"\(\s*(-?\d+)\s*\.\.\s*(-?\d+)\s*\)",
            RegexOptions.Compiled);

        private static readonly Regex SizeConstraintRegex = new Regex(
            @"SIZE\s*\(\s*(\d+)\s*\.\.\s*(\d+)\s*\)",
            RegexOptions.Compiled);

        // IMPORTS block: everything from IMPORTS keyword to the terminating semicolon
        private static readonly Regex ImportsBlockRegex = new Regex(
            @"\bIMPORTS\s+([\s\S]*?);",
            RegexOptions.Compiled);
        ...
        private static readonly Regex ModuleNameRegex = new Regex(
        ...
        {
            if (filePaths.Count == 0) return new List<MibFileInfo>();
            if (filePaths.Count == 1) return new List<MibFileInfo> { ParseFile(filePaths[0]) };
